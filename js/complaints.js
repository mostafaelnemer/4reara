var complaint_id= strings.complaint_id;
var no_complaint= strings.no_complaint;
            ...
            if(msg.success){
                ...
                if(msg.result.length){
                    msg.result.forEach(function(item){
                        ...
                        html+='<a href="javascript:void(0)" data-id="'+item.id+'" class="single-complaint"><div class="panel panel-default order-panel"> <div class="panel-heading"> <div class="order-title"> <div class="order-name pull-left"> <h3 class="panel-title"><span >'+complaint_id+'</span> #'+item.id+'</h3> <ul class="list-inline"> <li><i class="fa fa-calendar-alt"></i>'+formatDate(add_date)+'</li> <li><i class="fa fa-clock"></i>'+formatTime(add_date)+'</li> </ul> </div> <span class="order-status success pull-right"><i class="fa '+statusIcon(item.statues)+'"></i><span class="status-text"> '+item.statues+'</span></span> </div> </div><!-- end panel-heading --> </div></a>'
                    });
                }else{
                    html+='<div class="panel panel-default order-panel"> <div class="panel-heading"> <div class="order-title"> <div class="order-name"> <h3 class="panel-title text-center"><span>'+no_complaint+'</h3>  </div>  </div> </div><!-- end panel-heading --> </div>'
                }
